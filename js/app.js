window.app = {
	 ...
	 * netty服务后端发布的url地址
	 */
	nettyServerUrl: 'ws://172.20.10.9:8089/ws',
	// nettyServerUrl: 'ws://192.168.43.196/ws',
	
	/**
	 * 后端服务发布的url地址
	 */
	serverUrl: 'http://172.20.10.9:8080/v2',
	// serverUrl: 'http://192.168.43.196:8080/v2',
	
	/**
	 * 朋友圈媒体服务器的url地址
	 */
	// mediaServerUrl: 'http://192.168.43.196:8080',
	mediaServerUrl: 'http://172.20.10.9:8080',
	/**
	 * 图片服务器的url地址
	 */
	imgServerUrl: 'http://172.20.10.12:88/megetood/',
	...
	showToast: function(msg, type) {
		plus.nativeUI.toast(msg, 
			{icon: "/images/icon/" + type + ".png", verticalAlign: "center",duration:"1000"})
	},
	...
	saveUserChatHistory: function(myId, friendId, msg, flag) {
		...
		var chatKey = me._makeChatKey(myId,friendId);
		
		// 从本地缓存获取聊天记录是否存在
		var chatHistoryList = me.getUserChatHistory(myId,friendId)
	...
	// 生成保存聊天历史需要的key
	_makeChatKey(myId,friendId){
		return "chat-" + myId + "-" + friendId
	},
	...
	deleteUserChatHistory: function(myId, friendId) {
		var chatKey = this._makeChatKey(myId,friendId)
	...
	saveUserChatSnapshot: function(myId, friendId, msg, isRead) {
		var me = this
		var chatKey = me._makeChatSnapshotKey(myId)
	...
	// 生成保存聊天快照需要的key
	_makeChatSnapshotKey:function(myId){
		return "chat-snapshot" + myId
	},
